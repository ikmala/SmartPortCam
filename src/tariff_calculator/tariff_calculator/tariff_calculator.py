...
class TariffCalculatorNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter(
            "dimension_rules",
            json.dumps(DEFAULT_DIMENSION_RULES),
        )
        self.declare_parameter(
            "class_tariffs",
            json.dumps(DEFAULT_CLASS_TARIFFS),
        )
        self.declare_parameter("dimension_unit", "mm")
        ...
        self._unit_factor, self._unit_label = self._load_unit_parameter()
    ...
    def _load_dimension_rules(self) -> List[Dict[str, float]]:
        ...
        if isinstance(rules_param, str):
            try:
                rules_param = json.loads(rules_param)
            except json.JSONDecodeError:
                self.get_logger().warning(
                    "Parameter 'dimension_rules' tidak valid; kembali ke default."
                )
                rules_param = DEFAULT_DIMENSION_RULES
    ...
    def _load_class_tariffs(self) -> Dict[int, Dict[str, float]]:
        ...
        if isinstance(class_param, str):
            try:
                class_param = json.loads(class_param)
            except json.JSONDecodeError:
                self.get_logger().warning(
                    "Parameter 'class_tariffs' tidak valid; kembali ke default."
                )
                class_param = DEFAULT_CLASS_TARIFFS
    ...
    def _run_gui_loop(self) -> None:
        ...
        unit_text = self._unit_label
        self._length_var = tk.StringVar(value=f"Panjang: -- {unit_text}")
        self._width_var = tk.StringVar(value=f"Lebar: -- {unit_text}")
        self._height_var = tk.StringVar(value=f"Tinggi: -- {unit_text}")
            ...
            while True:
                ...
                if kind == "tariff":
                    ...
                    self._length_var.set(
                        f"Panjang: {self._format_dimension(dim.length)} {self._unit_label}"
                    )
                    self._width_var.set(
                        f"Lebar: {self._format_dimension(dim.width)} {self._unit_label}"
                    )
                    self._height_var.set(
                        f"Tinggi: {self._format_dimension(dim.height)} {self._unit_label}"
                    )
    ...
    def _dimension_callback(self, msg: VehicleDimension) -> None:
        ...
        if self._enable_gui:
            ...
            self.get_logger().info(
                "Kategori %s (class_id=%d) - Tarif %.0f %s | L=%.2f %s, W=%.2f %s, H=%.2f %s",
                category,
                msg.class_id,
                tariff,
                self._currency,
                self._format_dimension(msg.length),
                self._unit_label,
                self._format_dimension(msg.width),
                self._unit_label,
                self._format_dimension(msg.height),
                self._unit_label,
            )
    ...
    def _load_unit_parameter(self) -> Tuple[float, str]:
        """Return divisor and label for dimension display unit."""
        requested = str(self.get_parameter("dimension_unit").value).lower()
        if requested not in _UNIT_MAP:
            self.get_logger().warning(
                "Unit '%s' tidak dikenal; gunakan mm/cm/m. Menggunakan mm.",
                requested,
            )
            return _UNIT_MAP["mm"]
        return _UNIT_MAP[requested]

    def _format_dimension(self, value_mm: float) -> float:
        """Convert dimension from mm ke unit pilihan untuk display/log."""
        return value_mm / self._unit_factor
